        ...
        for col in range(n_samples):
            if col == 0:
                axes[0, col].set_ylabel(
                    "ID original", fontsize=9, fontweight="bold", color="steelblue"
                )
...
def _build_ddpm_timestep_grid(ctx: StageContext, metadata: DatasetMetadata) -> plt.Figure:
    ...
    if mode == "full":
        ...
        with torch.no_grad():
            _id_pairs = [
                ctx.model.reconstruct_at_t(
                    x_id[i : i + 1], t_values[i : i + 1], noise=noise_id[i : i + 1]
                )
                for i in range(len(keyframes))
            ]
            _ood_pairs = [
                ctx.model.reconstruct_at_t(
                    x_ood[i : i + 1], t_values[i : i + 1], noise=noise_ood[i : i + 1]
                )
                for i in range(len(keyframes))
            ]
...
def _build_ddpm_denoising_trajectory(ctx: StageContext, metadata: DatasetMetadata) -> plt.Figure:
    ...
    if mode == "full":
        ...
        for seed in range(n_runs):
            ...
            noise = torch.randn(
                x_shape.shape, generator=rng, device=ctx.device, dtype=x_shape.dtype
            )
            trajs.append(
                ctx.model.denoise_trajectory(
                    x_shape,
                    t_start=t_gen,
                    capture_timesteps=keyframes,
                    noise=noise,
                    from_noise=True,
                )
            )
    ...
    fig, axes = plt.subplots(
        2, n_cols, figsize=(textwidth * n_cols / 5, style.FIG_H1), squeeze=False
    )
