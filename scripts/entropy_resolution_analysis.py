...
print(f"{'Res':>5}  {'Clase':<22}  {'Entropía':>10}  {'Var local':>10}  {'Grad mag':>10}  {'Tipo':>6}")
print("-" * 72)
...
for res in RESOLUTIONS:
    ...
    for cls in ALL_CLASSES:
        ...
        results[res][cls] = {
            k: np.array([m[k] for m in metrics_list])
            for k in ("entropy", "local_var", "grad_mag")
        }
        ...
        print(f"{res:>5}  {CLASS_NAMES[cls]:<22}  {ent_m:>10.4f}  {var_m:>10.6f}  {grd_m:>10.6f}  {tag:>6}")
...
print(f"{'Res':>5}  {'Métrica':<12}  {'ID mean':>9}  {'OOD mean':>9}  {'Ratio':>7}  {'Dirección':>12}")
print("-" * 62)
...
for res in RESOLUTIONS:
    ...
    for metric in ("entropy", "local_var", "grad_mag"):
        ...
        print(f"{res:>5}  {metric:<12}  {id_mean:>9.5f}  {ood_mean:>9.5f}  {ratio:>7.3f}  {direction:>12}")
    print()
...
for ax, metric in zip(axes, metrics_labels):
    ...
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1.2, label="ratio=1 (igual complejidad)")
...
gs  = gridspec.GridSpec(len(RESOLUTIONS), len(ALL_CLASSES), figure=fig,
...
for i, res in enumerate(RESOLUTIONS):
    for j, cls in enumerate(ALL_CLASSES):
        ...
        ax.set_title(f"{CLASS_NAMES[cls][:12]}\n({res}px) μ={vals.mean():.2f}",
...
for k, (metric, label) in enumerate(metrics_labels.items()):
    ...
    bars   = ax.bar(x + k * w, vals, w, label=label, color=colors, alpha=0.85, edgecolor="k", linewidth=0.7)
...
best_res = max(RESOLUTIONS, key=lambda r: min(ratio_table[r][m] for m in ("entropy", "local_var", "grad_mag")))
